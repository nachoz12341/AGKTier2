...
int AGKSoundFileSeek(void *pUser, ogg_int64_t offset, int whence)
{
	...
	if ( whence == SEEK_SET ) pFile->Seek( (UINT)offset );
	else if ( whence == SEEK_CUR ) pFile->Seek( (UINT)offset + pFile->GetPos() );
	else if ( whence == SEEK_END ) pFile->Seek( pFile->GetSize() + (UINT)offset );
...
cMusicMgr::cMusic* cMusicMgr::m_pCurrentlyPlaying = UNDEF;

cMusicMgr::cMusicMgr()
{
	for ( UINT i = 0; i < MAX_MUSIC_FILES; i++ )
	{
		m_pMusicFiles[ i ] = UNDEF;
	}
	m_iStartID = 0;
	m_iEndID = 0;
	m_bLoop = true;
...
UINT cMusicMgr::AddFile( const uString &szFilename )
{
	UINT iID = m_iLastID;
...
void cMusicMgr::AddFile( UINT iID, const uString &szFilename )
{
...
UINT cMusicMgr::Exists( UINT iID )
{
...
void cMusicMgr::SetFileVolume( UINT iID, int volume )
{
...
UINT cMusicMgr::CurrentlyPlaying()
{
...
void cMusicMgr::RemoveFile( UINT iID )
{
	if ( iID < 1 || iID >= MAX_MUSIC_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
...
cSoundInst* cSoundMgr::m_pSounds = UNDEF;
cSoundInst* cSoundMgr::m_pUsedSounds = UNDEF;
UINT cSoundMgr::m_iLastID = 0;
...
void cSoundMgr::Init()
{
	for ( int i = 0; i < MAX_SOUND_FILES; i++ ) m_pSoundFiles[ i ] = UNDEF;
	m_pSounds = UNDEF;
	m_pUsedSounds = UNDEF;
...
UINT cSoundMgr::AddFile( const uString &szFilename, int iMaxInstances )
{
	UINT iID = m_iLastID + 1;
	if ( iID >= MAX_SOUND_FILES ) iID = 1;
...
void cSoundMgr::AddFile( UINT iID, const uString &szFilename, int iMaxInstances )
{
	uString szFile( szFilename );
	szFile.Replace( '\\', '/' );
	...
	UINT pos = oFile.GetPos() + (dwSize-16);
	oFile.Seek( pos );
...
UINT cSoundMgr::AddFile( const unsigned char *pData, UINT size, int iMaxInstances )
{
	UINT iID = m_iLastID + 1;
	if ( iID >= MAX_SOUND_FILES ) iID = 1;
...
void cSoundMgr::AddFile( UINT iID, const unsigned char *pData, UINT size, int iMaxInstances )
{
	if ( iID < 1 || iID >= MAX_SOUND_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
...
void cSoundMgr::AddOGGFile( UINT iID, const uString &szFilename, int iMaxInstances )
{
	uString szFile( szFilename );
	szFile.Replace( '\\', '/' );
...
void cSoundMgr::SaveFile( UINT iID, const uString &szFilename )
{
	uString szFile( szFilename );
	szFile.Replace( '\\', '/' );
	...
	UINT dwData = 0x46464952; // RIFF backwards
...
cSoundMgr::cSoundFile* cSoundMgr::GetFile( UINT iID )
{
	if ( iID < 1 || iID >= MAX_SOUND_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
...
UINT cSoundMgr::PlayInstance( UINT iID, int iVol, int iLoop, int iPriority )
{
	if ( iID < 1 || iID >= MAX_SOUND_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
...
/*
void cSoundMgr::PauseAll( UINT iID )
{
...
void cSoundMgr::ResumeAll( UINT iID )
{
	if ( iID < 0 || iID >= MAX_SOUND_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
...
void cSoundMgr::StopAll( UINT iID )
{
	if ( iID >= MAX_SOUND_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
...
void cSoundMgr::DeleteFile( UINT iID )
{
	if ( iID >= MAX_SOUND_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
	...
	if ( iID == 0 )
	{
		for ( int i = 0; i < MAX_SOUND_FILES; i++ ) 
		{
			...
			m_pSoundFiles[ i ] = UNDEF;
		}
	}
	else
	{
		if ( m_pSoundFiles[ iID ] ) delete m_pSoundFiles[ iID ];
		m_pSoundFiles[ iID ] = UNDEF;
	}
}

UINT cSoundMgr::CountInstances( UINT iID )
{
	if ( iID < 1 || iID >= MAX_SOUND_FILES ) 
	{
#ifdef _AGK_ERROR_CHECK
...
UINT cSoundMgr::Exists( UINT iID )
{
