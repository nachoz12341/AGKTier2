...
{
	for( UINT i = 0; i < m_iNumMainImages; i++ )
	{
...
int AGKSizedFont::AddToMainImage( AGKFontImage* pNewFontImage )
{
	...
	m_pCurrImage->LoadSubRegionFromData( m_iCurrX+offset, m_iCurrY+offset, pNewFontImage->m_iDataWidth, pNewFontImage->m_iDataHeight, pNewFontImage->m_pData, 2 );
...
void AGKFont::RebuildImages()
{
	AGKSizedFont *pSizedFont = m_SizedFonts.GetFirst();
	while( pSizedFont )
	{
		pSizedFont->RebuildImages();
		pSizedFont = m_SizedFonts.GetNext();
	}
}

void AGKFont::RebuildAllFontImages()
{
	AGKFont *pFont = g_pAllFonts;
	while( pFont )
	{
		pFont->RebuildImages();
		pFont = pFont->m_pNextFont;
	}
}
