...
void PlatformAppQuit()
{
	...
	jmethodID method = lJNIEnv->GetStaticMethodID( classHelper, "QuitApp","(Landroid/app/Activity;)V");
	if ( !method ) agk::Warning("Failed to get method QuitApp");
...
extern "C"
{
	...
	void startcrashcheck( ANativeActivity* pActivity )
	{
		const char *internal = pActivity->internalDataPath;
		char path[ 1024 ];
		if ( internal )
		{
			strcpy( path, internal );
		}
		else
		{
			strcpy( path, "/sdcard/" );
		}
		strcat( path, "/crashcheck.txt" );

		remove("/sdcard/crashreport.txt");

		FILE *pFile = fopen(path,"rb");
		if ( pFile )
		{
			// problem, this should not exist on a clean exit
			fclose(pFile);
			agk::Warning( "Detected possible crash" );
			...
			if ( !pActivity ) agk::Warning("Failed to get activity pointer");
			...
			jmethodID methodGetLog = lJNIEnv->GetStaticMethodID( classHelper, "GenerateCrashReport","(Landroid/app/Activity;)V");
			if ( !methodGetLog ) agk::Warning("Failed to get method GenerateCrashReport");

			lJNIEnv->CallStaticVoidMethod( classHelper, methodGetLog, lNativeActivity );

			vm->DetachCurrentThread();
		}

		agk::Warning( "Creating crash check file" );
		pFile = fopen(path,"wb");
		if ( pFile ) fclose(pFile);
		else agk::Warning( "Failed to create crash check file" );
	}

	void endcrashcheck( ANativeActivity* pActivity )
	{
		agk::Warning( "Removing crash check file" );
		const char *internal = pActivity->internalDataPath;
		char path[ 1024 ];
		if ( internal )
		{
...
/*
	...
	void pauseapp()
	{
		...
		jmethodID method = lJNIEnv->GetStaticMethodID( classHelper, "OnStop","(Landroid/app/Activity;)V");
		if ( !method ) agk::Warning("Failed to get method OnStop");
		else lJNIEnv->CallStaticVoidMethod( classHelper, method, lNativeActivity );
	...
	void onstart(void* ptr)
	{
		...
		jint lFlags = 0;
		JNIEnv* lJNIEnv = act->env;
		JavaVM* vm = act->vm;
		vm->AttachCurrentThread(&lJNIEnv, NULL);

		if ( !act ) agk::Warning("Failed to get activity pointer");
		jobject lNativeActivity = act->clazz;
		if ( !lNativeActivity ) agk::Warning("Failed to get native activity pointer");
		...
		jmethodID method = lJNIEnv->GetStaticMethodID( classHelper, "OnStart","(Landroid/app/Activity;)V");
		if ( !method ) agk::Warning("Failed to get method OnStart");
		else lJNIEnv->CallStaticVoidMethod( classHelper, method, lNativeActivity );
	...
	{
		if ( key == AKEYCODE_MENU ) menuPressed = 1;
