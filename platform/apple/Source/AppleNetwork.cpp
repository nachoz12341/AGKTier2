...
bool UDPManager::SendPacket( const char *IP, const AGKPacket *packet )
{
	...
	ssize_t result = sendto( m_socket, packet->GetBuffer(), packet->GetPos(), 0, (sockaddr*)&addr, length );
...
bool UDPManager::RecvPacket( char *fromIP, AGKPacket *packet )
{
	...
	ssize_t result = recvfrom( m_socket, packet->GetRaw(), AGK_NET_PACKET_SIZE, 0, (sockaddr*)&addr, &length );
...
bool AGKSocket::SendString( const char *s )
{
	...
	if ( !SendUInt( (UINT)length ) ) return false;
	if ( length > 0 ) return SendData( s, (UINT)length );
	return true;
}
...
int AGKSocket::RecvData( char* data, int length )
{
	...
	ssize_t result = recv( m_client, data, length, 0 );
	if ( result == SOCKET_ERROR || result == 0 )
	{
...
int AGKSocket::RecvString( uString &s )
{
	...
	ssize_t result = recv( m_client, buf, length, 0 );
	ssize_t sofar = result;
	while ( result != SOCKET_ERROR && result > 0 && sofar < (int)length )
	{
		s.AppendN( buf, (UINT)result );
...
char AGKSocket::RecvChar()
{
	...
	char c = 0;
	ssize_t result = recv( m_client, &c, 1, 0 );
...
int AGKSocket::RecvInt()
{
	...
	while ( received < 4 )
	{
		ssize_t result = recv( m_client, ((char*)&i)+received, 4-received, 0 );
		if ( result == SOCKET_ERROR || result == 0 ) 
		{
			m_bDisconnected = true;
			return 0;
...
UINT AGKSocket::RecvUInt()
{
	...
	while ( received < 4 )
	{
		ssize_t result = recv( m_client, ((char*)&u)+received, 4-received, 0 );
		if ( result == SOCKET_ERROR || result == 0 ) 
		{
			m_bDisconnected = true;
			return 0;
...
float AGKSocket::RecvFloat()
{
	...
	while ( received < 4 )
	{
		ssize_t result = recv( m_client, ((char*)&f)+received, 4-received, 0 );
		if ( result == SOCKET_ERROR || result == 0 ) 
		{
			m_bDisconnected = true;
			return 0;
...
int cNetworkListener::AcceptConnections( const char *szIP, UINT port )
{
	...
	{
        uString err;
        err.Format("Failed to bind listening socket: %d", errno);
		agk::Warning( err );
		close( m_socket );
		m_socket = INVALID_SOCKET;
		return 0;
