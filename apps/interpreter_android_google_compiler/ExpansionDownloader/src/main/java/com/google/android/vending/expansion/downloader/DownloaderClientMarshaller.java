...
public class DownloaderClientMarshaller {
    ...
    static IDownloaderClient mHackedInCallbackClient = null;
    ...
    private static class Proxy implements IDownloaderClient {
        ...
        public void onDownloadStateChanged(int newState) {
        	Log.e("Download Client Manager Proxy", "State Changed: "+newState);
            Bundle params = new Bundle(1);
            params.putInt(PARAM_NEW_STATE, newState);
            send(MSG_ONDOWNLOADSTATE_CHANGED, params);
            if ( mHackedInCallbackClient != null )
            {
            	mHackedInCallbackClient.onDownloadStateChanged(newState);
            }
            else Log.e( "Download Client", "State client is null" );
        }
        ...
        public void onDownloadProgress(DownloadProgressInfo progress) {
            ...
            if ( mHackedInCallbackClient != null )
            {
            	mHackedInCallbackClient.onDownloadProgress(progress);
            }
            else Log.e( "Download Client", "Progress client is null" );
        }
        ...
        private void send(int method, Bundle params) {
            ...
            m.setData(params);
            try {
            	Log.e("Client Proxy","Service Messager: "+mServiceMessenger.toString()+" message: "+m.toString());
                mServiceMessenger.send(m); // message is never received??
            } catch (RemoteException e) {
                e.printStackTrace();
                Log.e("Proxy Send",e.toString());
            }
        }
    ...
    private static class Stub implements IStub {
        ...
        // Why doesn't this work?? It never receives messages
        final Handler mHandler = new Handler() {
            @Override
            public void handleMessage(Message msg) {
            	//Log.e("Messenger","Msg: "+msg.toString());
                switch (msg.what) {
        ...
        public Messenger getMessenger() {
        	//String info = String.format( "Got Messager: %s", mMessenger.toString());
        	//Log.e("Client Stub",info);
            return mMessenger;
        }
    }
